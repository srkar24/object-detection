 ...
 *
 * This function calculates the error between the measured distances of the left and right sensors
 * and adjusts the motor duty cycles to turn towards any detected object. It also ensures that the
 * duty cycles remain within the defined range and that the robot follows objects within a specific range.
 * It is used for object following.
 *
 * @param None
 *
 * @return None
...
#ifndef DEBUG_ACTIVE
    // Apply the updated PWM duty cycle values to the motors
    if ((Converted_Distance_Left > TOO_CLOSE_DISTANCE)&&(Converted_Distance_Right > TOO_CLOSE_DISTANCE)&&(Converted_Distance_Center > TOO_CLOSE_DISTANCE)){
        Motor_Stop();
    }
    else{
    Motor_Forward(Duty_Cycle_Left, Duty_Cycle_Right);
 ...
 *
 * This function calculates the error between the measured distances of the left and right sensors
 * and adjusts the motor duty cycles to back away from any detected object. It also ensures that the
 * duty cycles remain within the defined range and that the robot stops if objects are detected beyond a specific range.
 * It is used for object avoidance.
 *
 * @param None
 *
 * @return None
